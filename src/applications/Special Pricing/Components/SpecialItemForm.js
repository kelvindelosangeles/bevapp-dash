...
const SpecialItemForm = ({ specialPrice, beverages, toggleActiveState, onChange, state }) => {
    return (
        <React.Fragment>
            <p>{specialPrice.id}</p>
            <p>$ {beverages[specialPrice.id].price}</p>
            <SPInput type='number' name={specialPrice.id} onChange={onChange} value={state[specialPrice.id].price} min={0} max={100} />
            <p>$ {OrdersModel.CalcMargin(beverages[specialPrice.id].price, specialPrice.price)}</p>

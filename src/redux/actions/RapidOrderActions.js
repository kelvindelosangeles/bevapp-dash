...
export const newOrder = (customer) => (dispatch, getState) => {
    dispatch({
        type: "NEW_ORDER",
        payload: {
            customer,
...
export const editOrder = (order, history) => (dispatch, getState) => {
...
export const deleteOrder = (order, firestore, closeOrderPreview) => (dispatch, getState) => {
...
export const saveToDrafts = (firestore) => (dispatch, getState) => {
...
export const deleteDraft = (id, history) => (dispatch) => {
    console.log("the ID to remove", id);
    try {
        store.remove(id);
        history.push("/dashboard");
    } catch (error) {
        console.log("There was an error deleting the draft", error);
    }
};
...
export const updateCustomer = (newCustomer) => (dispatch, getState) => {
    // FIXME:CLEAN UP, this is a combination of old actions and new actions in one
